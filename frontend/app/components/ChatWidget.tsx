...
export default function ChatWidget() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (res.data.error) {
                setMessages((prev) => [...prev, { role: "bot", text: `❌ API Error: ${res.data.error} (Hint: ${res.data.hint})` }]);
            }
            ...
            // 4. Agar khali response aaya
                ...
                setMessages((prev) => [...prev, { role: "bot", text: "⚠️ Server sent empty response." }]);
            }

        } catch (error: any) {
            console.error("Connection Error:", error);
            setMessages((prev) => [...prev, { role: "bot", text: "🚫 Connection Failed! Check if Backend is running." }]);
        } finally {
            setLoading(false);
        }
    };
